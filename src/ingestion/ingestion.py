...
def ingest_data(config_path: str) -> None:
    ...
    spark._jsc.hadoopConfiguration().set(
        ...
        config["azure"]["connection_string"].split("AccountKey=")[1].split(";")[0]
    )
    
    try:
        data_path = f"wasbs://{config['azure']['container']}@{config['azure']['storage_account']}.blob.core.windows.net/data/retail_price.csv"
